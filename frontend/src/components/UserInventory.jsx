...
export default function UserInventory() {
    ...
    useEffect(() => {
        const fetchInventory = async () => {
            ...
            try {
                const res = await fetch('http://127.0.0.1:8000/gamification/inventory', {
        ...
        return () => window.removeEventListener('inventory-updated', fetchInventory);
    }, []);

    if (loading) return <div className="text-center text-[#7F5539] text-xs">Cargando mochila...</div>;
    ...
    return (
        <div className="px-4 pb-8">
            <h3 className="text-[#3C2A20] font-bold mb-3 ml-2 flex items-center gap-2">
                🎒 Mis Cosas
                <span className="bg-[#3C2A20] text-white text-[10px] px-2 py-0.5 rounded-full">{items.length}</span>
            </h3>
            
            <div className="space-y-2">
                {items.map((item) => (
                    <div key={item.id} className="bg-white p-3 rounded-xl border border-[#E6CCB2] flex justify-between items-center shadow-sm">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-[#3C2A20] font-bold text-sm">{item.item_name}</p>
                                <p className="text-[#7F5539] text-xs">Comprado el {new Date(item.acquired_at).toLocaleDateString()}</p>
                            </div>
                        </div>
